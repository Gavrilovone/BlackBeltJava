 ...
 * 
 * очень важно, чтобы ключ был immutable, для этого поля можно пометить final и сам класс тоже final, чтобы он не мог иметь наследников.если этого не сделать
 * то в случае изменения объекта(одного из его полей например) мы не сможем найти его в нашем HashMape, так как у ключа будет уже другой hashcode, поэтому
 * рекомендуется, чтобы объект который стоит на месте ключа был неизменяемый
 * 
 * HashMap это коллекция которая является not synchronized и поэтому ее не стоит использовать в multithreaded(многопоточность) программировании, для этого есть
 * коллекция concurrentHashMap (concurrent - одновременный)
 */
public class Interface_HashMap {
    public static void main(String[] args) {
        ...
        /**
         * метод entrySet - entry это интерфейс внктренний для Map который имплементируется нашим классом внутренним для Map Node(который содержит ключ значение
         * хэш ключа и ссылку на следующий элемент), поэтому с помощью entry мы можем добраться до ключа и до значения
         */
